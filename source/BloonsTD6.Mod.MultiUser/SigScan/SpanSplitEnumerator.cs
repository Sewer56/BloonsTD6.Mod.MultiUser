...
public ref struct SpanSplitEnumerator<TSpanType>
{
    /// <summary>
    ...
    /// </summary>
    public TSpanType[] Current { get; private set; }

    /// <summary>
    /// The original span this struct was instantiated with.
    /// </summary>
    private TSpanType[] _original;
    ...
    {
        var index = Array.IndexOf(_original, SplitItem);
        if (index == -1)
        {
            if (_reachedEnd)
                return false;
    ...
    public SpanSplitEnumerator(TSpanType[] item, TSpanType splitItem)
    {
        _original = item;
        Current = _original;
        SplitItem = splitItem;
